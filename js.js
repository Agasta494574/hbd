...
// NOTE: Karena kita tidak memiliki elemen-elemen UI ini di HTML kita,
// appNodes akan mencari null. Ini akan menyebabkan error jika renderApp dipanggil
// dengan asumsi semua elemen ada.
// Kita akan membuat objek appNodes kosong untuk menghindari error.
// Jika Anda ingin UI asli, Anda harus menambahkan elemen HTML sesuai CodePen.
const appNodes = {}; // <-- MODIFIKASI: Buat objek kosong
// appNodes = { /* appNodes dari CodePen asli di sini jika ingin UI */ };

// Mengganti semua querySelector menjadi placeholder atau null
// Ini diperlukan karena kita tidak punya semua elemen UI dari CodePen asli
['stageContainer', 'canvasContainer', 'controls', 'menu', 'menuInnerWrap', 'pauseBtn', 'pauseBtnSVG', 'soundBtn', 'soundBtnSVG', 'shellType', 'shellTypeLabel', 'shellSize', 'shellSizeLabel', 'quality', 'qualityLabel', 'skyLighting', 'skyLightingLabel', 'scaleFactor', 'scaleFactorLabel', 'autoLaunch', 'autoLaunchLabel', 'finaleModeFormOption', 'finaleMode', 'finaleModeLabel', 'hideControls', 'hideControlsLabel', 'fullscreenFormOption', 'fullscreen', 'fullscreenLabel', 'longExposure', 'longExposureLabel', 'helpModal', 'helpModalOverlay', 'helpModalHeader', 'helpModalBody', 'helpModalCloseBtn']
.forEach(key => {
    appNodes[key] = document.querySelector(`.${key}`) || {
...
// Memastikan elemen penting seperti canvasContainer dan stageContainer tersedia
appNodes.canvasContainer = document.querySelector('.canvas-container') || { style: {} };
appNodes.stageContainer = document.querySelector('.stage-container') || { classList: { remove: () => {} } };
appNodes.loadingInitStatus = document.querySelector('.loading-init__status') || { textContent: '' };


// First render is called in init()
function renderApp(state) {
    // Karena kita menyembunyikan UI, banyak kode di sini akan diabaikan atau dijalankan pada objek placeholder
    // Ini membantu mencegah error jika elemen UI tidak ditemukan.
    ...
    // Asign nilai ke elemen UI jika ada, jika tidak, objek placeholder akan mengabaikannya
...
// Mengomentari atau menghapus event listener karena tidak ada elemen UI yang sesuai di HTML kita
/*
...
function init() {
	...
	if (document.querySelector('.loading-init')) {
	...
	function setOptionsForSelect(node, options) {
		if (node) { // Pastikan node ada sebelum memanipulasinya
            // MODIFIKASI: Gunakan Array.prototype.reduce dengan benar
            // Pastikan 'options' adalah array of objects dengan {value, label}
			node.innerHTML = options.reduce((acc, opt) => acc += `<option value="${opt.value}">${opt.label}</option>`, '');
		}
	}

	// shell type
    // MODIFIKASI: Buat array of objects untuk shellType
	let shellTypeOptions = shellNames.map(opt => ({ value: opt, label: opt }));
	setOptionsForSelect(appNodes.shellType, shellTypeOptions);
	
	// shell size
    // MODIFIKASI: Buat array of objects untuk shellSize
	let shellSizeOptions = ['3"', '4"', '6"', '8"', '12"', '16"'].map((opt, i) => ({ value: String(i), label: opt }));
	setOptionsForSelect(appNodes.shellSize, shellSizeOptions);
...
function handlePointerStart(event) {
	...
	// Bagian ini adalah untuk kontrol UI yang tidak kita implementasikan di HTML ini.
	// Jika ingin, Anda bisa membuat tombol pause, sound, menu di HTML Anda
	// dan mengaitkannya dengan fungsi togglePause/toggleSound/toggleMenu.
	// Untuk saat ini, kita akan melewati logika ini agar klik langsung meluncurkan kembang api.
	/*
	if (event.y < btnSize) {
		if (event.x < btnSize) {
			togglePause();
	...
	*/
	...
	// HANYA Meluncurkan kembang api jika app sedang berjalan
	if (!isRunning()) return;
	
	// Jika Anda ingin speed bar berfungsi, biarkan kode ini.
	// Untuk saat ini, kita fokus pada peluncuran.
	/*
	...
	*/
    // Modifikasi: Selalu luncurkan shell jika klik terjadi di kanvas
    if (event.onCanvas) {
...
function handleResize() {
    ...
    // MODIFIKASI: Pastikan elemen stageContainer ada
...
function createBurst(count, particleFactory, startAngle=0, arcLength=PI_2) {
	...
	// MODIFIKASI: Pastikan C_HALF tidak 0
	const adjustedC_HALF = C_HALF || 1; 

	for (let i=0; i<=adjustedC_HALF; i++) { // Loop ini perlu divalidasi agar tidak menghasilkan NaN jika C_HALF terlalu kecil
		const ringAngle = i / adjustedC_HALF * PI_HALF;
		...
		// MODIFIKASI: Hindari loop tak terbatas jika partsPerArc terlalu kecil atau NaN
		if (!Number.isFinite(partsPerArc) || partsPerArc <= 0) {
            continue; // Lewati jika tidak ada bagian untuk dibuat
        }
...
function setLoadingStatus(status) {
	// MODIFIKASI: Pastikan appNodes.loadingInitStatus ada sebelum mencoba mengakses textContent
	if (appNodes.loadingInitStatus) {
		appNodes.loadingInitStatus.textContent = status;
	}
}

// Untuk proyek ini, kita akan menginisialisasi langsung tanpa preload audio
// Ini menghilangkan potensi masalah loading audio
init(); 

// MODIFIKASI: Mengomentari atau menghapus blok preload audio asli
/*
if (IS_HEADER) {
	init();
} else {
